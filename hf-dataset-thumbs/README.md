...
**108,389 pre-cropped thumbnail images — one per listing — from 46 volumes of mid-20th-century African American travel guides (1930–1966).** Each image is a small snippet of a scanned directory page cropped down to a single business or lodging listing: the exact region a traveler would have read.
...
Its purpose is practical: it lets the companion web explorer show the source snippet for every one of ~109k listings **without sending live requests to the New York Public Library or Library of Congress image servers**. Browse it in context here: <https://hadro.github.io/green-books/all-volumes>.

## What's in here

- **Format:** WebP, quality 75, **400 px wide** (height varies with the listing), averaging ~3.9 KB per image; ~421 MB total.
- **Layout:** content-addressed and sharded — `<tid[:2]>/<tid>.webp`, where `tid` is the first 12 hex characters of `sha1(canvas_fragment)`. 256 shard folders (`00/` … `ff/`), ~420 files each. There is no per-file metadata table in this repo — the filename *is* the key back to the tabular dataset.
...
| | Count |
|---|---:|
| Listings in the companion dataset | 109,163 |
| **Thumbnails here** | **108,389** (99.3%) |
| Volumes | 46 |
...
All 24 Green Book editions and all six companion publications are represented, including the **1946 Green Book digitized by the Library of Congress** (item [2016298176](https://www.loc.gov/item/2016298176/)); every other volume is from **NYPL**.
...
Unlike the *facts* in the companion dataset (which are uncopyrightable directory data under *Feist v. Rural Telephone*, 499 U.S. 340), these files are **image crops of the source scans**, so the scans' own status matters directly. Verified against the NYPL API (45 NYPL volumes) and the LOC item record (1 LOC volume):

- **45 of 46 volumes** are **Public Domain in the United States** — 44 NYPL volumes (`NoC-US`, NYPL status `PDREN`) plus the **1946 Green Book** from the Library of Congress (no known publication restrictions, `NoC-US`). Crops of these pages are in the public domain.
- **1 of 46 — *Travelguide 1957*** — is an **in-copyright orphan work** (`InC-RUU`, NYPL status `ICORPHAN`): NYPL found a copyright notice, could not locate a rights-holder, and released the scan as an orphan work. Its listings' *facts* are free to reuse, but its page *scans* are not public domain, so the ~2,483 thumbnails cropped from it are **derivatives of an orphan-work scan**. They are included and flagged here for parity with the text dataset; downstream users who need a strictly-public-domain image set should exclude that volume. Per-volume rights are in [`volume_rights.csv`](https://huggingface.co/datasets/hadro/green-books-travel-guides/blob/main/volume_rights.csv) in the companion dataset.
...
@dataset{green_book_thumbnails,
  ...
  author = {Hadro, Josh},
  year   = {2026},
  note   = {108,389 per-listing snippet images cropped from 46 digitized volumes, New York Public Library and Library of Congress Digital Collections},
